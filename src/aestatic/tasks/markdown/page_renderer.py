...
class PageRenderer(mistune.HTMLRenderer):
    ...
    def link(self, text: str, url: str, title=None):
        if url.startswith("http"):  # catch all external links
        ...
        if url.partition('#')[0].endswith("html") or url.startswith("#"):
            return f'<a href="{self.safe_url(url)}">{text}</a>'  # all internal html links

        # catch all non-html links
